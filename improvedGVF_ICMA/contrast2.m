...
ud = 0.6; % 纵荡速度相同
V = 0.4;  % 流速相同
wvc = 0.12;  % 流的方向角速度（有旋流）
psi0 = 90*pi/180;  % 初始艏向角
dp = 0.26;
B = [1 1; dp -dp];
for k=1:Ns
   t = (k-1)*ts;
...
%    end
   if t>=50
       Vc = [V*sin(wvc*t) V*cos(wvc*t)]';
   end
   % 外界干扰
   tau_w = [15*sin(0.3*t)*cos(0.2*t)+15 15*cos(0.3*t)*sin(0.2*t)+10 ...
            10*cos(0.3*t)*sin(0.2*t)+10]';
   % 无人艇模型
   ...
   % 海流观测器
   [ vchat,vchat_dot,phat, phat_dot] = observer2( p,u,psi,ts );
   
   % 向量场制导
   [rd,e,xd,yd,theta,e_alphap,Xd_dot]  = GVFguidance2( pc,ud,vchat,vchat_dot,phat,phat_dot,psi,ts );

   % NN controller
   wd = [ud,rd]'; w = [u,r]';
   F = [fu,fr]';
   [Fhat, lambda, Tc ] = NNcontroller( wd,w,T,Tc,F,ts );
   lambdau = lambda(1); lambdar = lambda(2);
   fuhat = Fhat(1); frhat = Fhat(2);
...
fontsize = 12; fontname = 'TimesNewRoman'; linewid = 1.5; fontweight = 'bold';
...
h1=legend('desired path','trajectory of ONVF','trajectory of NVF');
...
set(h1,'Interpreter','latex'); set(h1,'FontSize',fontsize); set(h1,'FontName',fontname);
set(h1,'FontWeight',fontweight);
h1=ylabel('kinematic disturbances estimation (m/s)');
set(h1,'Interpreter','latex'); set(h1,'FontSize',fontsize); set(h1,'FontName',fontname);
set(h1,'FontWeight',fontweight);
box on
hold off
...
figure(3);
subplot(2,1,1);
plot(test1.t,test1.fu,'r-',test1.t,test1.fuhat,'b--','linewid',linewid);
...
set(h1,'Interpreter','latex'); set(h1,'FontSize',fontsize); set(h1,'FontName',fontname);
set(h1,'FontWeight',fontweight);
axes('Position',[0.2,0.75,0.2,0.15]); % 生成子图   
plot(test1.t,test1.fu,'r-',test1.t,test1.fuhat,'b--','linewid',linewid); % 绘制局部曲线图                        
xlim([0,6]); % 设置坐标轴范围  
ylim([-1.5,0.5]);
set(gca,'xTick',0:2:6);
...
box on
subplot(2,1,2);
plot(test1.t,test1.fr,'r-',test1.t,test1.frhat,'b--','linewid',linewid);
...
set(h1,'FontWeight',fontweight);
box on
axes('Position',[0.2,0.16,0.2,0.15]); % 生成子图   
plot(test1.t,test1.fr,'r-',test1.t,test1.frhat,'b--','linewid',linewid); % 绘制局部曲线图                                              
xlim([0,6]); % 设置坐标轴范围  
ylim([-4,4]);
set(gca,'xTick',0:2:6);
set(gca,'yTick',-4:2:4);
...
plt1 = plot3(test1.xd,test1.yd,test1.theta,'r-','linewid',linewid);
plt2 = plot3(test1.x,test1.y,test1.theta,'b--','linewid',2.5);
z = zeros(length(test1.theta),1);
plt3 = plot3(test1.xd,test1.yd, z,'g-','linewid',linewid);
plt4 = plot3(test1.x,test1.y,z,'m--','linewid',2.5);
h1 = legend([plt1,plt2,plt3,plt4],{'virtual path$$\ \bar{\eta}_{p}$$','virtual trajectory$$\ \bar{\eta}$$',...
            'real path$$\ \eta_{p}$$','real trajetory$$\ \eta$$'});
set(h1,'Interpreter','latex'); set(h1,'FontSize',fontsize); set(h1,'FontName',fontname);
set(h1,'FontWeight',fontweight);
for k=1:2000:Ns
    plot3([test1.x(k) test1.x(k)],[test1.y(k) test1.y(k)],[test1.theta(k) z(k)],'-k.','linewid',2,'markersize',25);
end
box on
