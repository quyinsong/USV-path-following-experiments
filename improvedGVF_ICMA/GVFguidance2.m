function [rd,e,xd,yd,y,e_alphap,Xd_dot]  = GVFguidance2( pc,ud,vchat,vchat_dot,p,p_dot,psi,ts )
%GVFGUIDANCE wd  = GVFguidance( Ud,Vchat,Vchat_dot,p,p_dot,X,ts )
%   向量场制导，返回期望转艏率
% 海流侧滑速度均未知，设计观测器得到其估计值
persistent theta k1 k2 kw L D
if isempty(theta)
   theta = 0; 
   k1 = 0.6; k2 = 0.6;
   kw = 5;
...
% 常用变量计算
mn = [cos(psi) sin(psi)]'; mc = ud*mn+vchat;
if norm(mc)==0
    mc = ud*mn;
end %               防止发生奇异
mcn = mc/norm(mc); 
...
rd1 = Xd_dot-kw*mcn'*E*mpdn;
rd = (norm(mc)*rd1+mcn'*E*vchat_dot)/(ud*mcn'*mn);
