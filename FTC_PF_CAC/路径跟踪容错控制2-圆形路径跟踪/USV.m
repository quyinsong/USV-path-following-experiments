function [x, xdot, Tf, Hx, Fx] = USV( x, T, tau_w, ts, t, tf )
%USV USV model 
...
for i = 1:2
    if t>=15 && t<35
        a = [1 1]'; beta = [1 1]'; zeta = [0 0]';
    end
    if t>=35 && t<55
        a = [1 1]'; beta = [1 99]'; zeta = [0 0]';
    end
    if t>=55 && t<75
        a = [1 1]'; beta = [99 1]'; zeta = [0.05*sin(0.8*t) 0.05*cos(0.8*t)]';
    end
    if t>=75
        a = [0 0]';
    end
...
R = [cos(psi) -sin(psi) 0
     sin(psi) cos(psi)  0
     0        0         1];
miu_dot = M\(-D*miu-C*miu+tau+tau_w+DeltaFx);
eta_dot = R*miu;
