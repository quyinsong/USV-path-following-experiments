function [xd, yd, psid, psid_dot, psip, xe, ye, w] = LOS( pc, k1, delta, eta, nu, v_dot, ud, w, ts )
...
if nargin ~=9,error('input number must be 10!');end
...
Xp = psi+betad-psip;
psie = psi-psip;
up = Ud*cos(Xp) + u_tilde*cos(psie)+k1*xe;   % 路径虚拟参考点的切向速度
wdot =  up / sqrt(xd_dw^2 + yd_dw^2);  % 路径参数的更新
w = euler2(wdot, w, ts);

psid = psip-betad-atan(ye/delta); 

ye_dot = u*sin(psie)+v*cos(psie)-kc*up*xe;
phi = Ud/sqrt(ye^2+delta^2);
betad_dot = ud*v_dot/Ud^2;
atanye_dot = ye_dot*phi*delta/Ud;
psid_dot = kc*up-betad_dot-atanye_dot;
