...
%           -1.9936*u*v-2.0081*r];
    ...
    switch flag
        case 1
            miu1_hat  = DynamicSystem_( miu1_hat, miu1, Hx_hat, Q_hat,B*Tc1 ,ts, B ,Fx); % dynamic system
            Hx_hat = RBFNN( miu1,B*Tc1,miu1_hat, ts, B );  % rbfnn estimate Hx
            [Q_hat,z] = DiturbanceObserver( z, Hx_hat, miu1, B*Tc1, Fx , B, ts); % diturbances observer
        case 0
            Hx_hat = [0 0]';
    end
    ...
    % estimate value
    xout(k,:) = [t x' Tc1' Tf' xd yd xe ye D_tilde' norm(D_tilde) flag miu1_hat' Hx_hat' Hx'];
end
t = xout(:,1);
u = xout(:,2);
v = xout(:,3);
