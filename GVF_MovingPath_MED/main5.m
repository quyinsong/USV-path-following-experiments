...
for k=1:Ns
   ...
   % Initialize
   if t==0
       psi = -30*pi/180; x = [0 0 0 35 -3 psi]'; tau_c = [0 0]';
       Ud = 0.8; dp = 0.26; B = [1 1;dp -dp];
       xT = 0; yT = 0; alphaT = 0*pi/180;
       kr = 0.4; kro = 0.001; vT = 0;
       Vc = [0 0]'; 
...
%    H = [2 0; 0 2];
% 
   phi = yr-20*sin(2*pi*xr/80);
   n = [ -20*2*pi*cos(2*pi*xr/80)/80 1]'; tau = -[-n(2) n(1)]';
   H = [20*(2*pi)^2*sin(2*pi*xr/80)/6400 0; 0 0];
...
%        uc(i) = uc1(i);
       ...
       if psir <= ci
           f1 = 0; f2 = 1;
       elseif psir < 0
           l1 = 100; l2 = 1;  % l1 越大则避障优先级越高 l2 越大则路径跟踪优先级越高
