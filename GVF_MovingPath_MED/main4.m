...
%            10 15 2 1; 4 15 2 1; 20 15 3 2]; % 障碍物的横纵坐标、反应域、拒绝域
       O = [30 10 8 3; 40 42 6 2; 5 28 8 3];
%          O = [10 10 0.1 0.01];
...
figure(1); hold on
box on
xrange=[-10 10 55]; yrange = [-10 10 55];
...
plt1 = plot(0,0,'k-','linewid',2);
plt2 = plot(0,0,'b--','linewid',2);
plt3 = plot(0,0,'r--','linewid',2);
plt4 = plot(0,0,'m--','linewid',1);
% 
% h1=legend([plt1(1),plt2(1)],...
%     {'Obstacle','target path'});
% set(h1,'Interpreter','latex'); set(h1,'FontSize',fontsize); set(h1,'FontName',fontname);
% set(h1,'FontWeight',fontweight);set(h1,'Location','SouthEast');
% set(h1,'Orientation','horizon');
% set(h1,'box','off');
% ah1 = axes('position',get(gca,'position'),'visible','off');
% h2=legend(ah1,[plt3(1),plt4(1)],...
%     {'trajectory','enclocsing path'});
% set(h2,'Interpreter','latex'); set(h2,'FontSize',fontsize); set(h2,'FontName',fontname);
% set(h2,'FontWeight',fontweight);set(h2,'Location','SouthEast');
...
F=MovieXY([x1 xT1],[y1 yT1],0.0001,{'o','*'},linecolor);
